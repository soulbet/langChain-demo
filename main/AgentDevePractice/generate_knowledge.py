...
**输出格式和内容要求（必须100%遵守）**：
1. 全文只允许使用普通文本、标点符号和代码块。**绝对不能出现任何 LaTeX 命令或标记**，包括：
   - 禁止 `\(`、`\)`、`\[`、`\]`
   - 禁止 `\begin`、`\end`
   - 禁止 `\frac`、`\log`、`\ln` 等任何以 `\` 开头的命令
   - 禁止 `_` 和 `^` 用于上下标（请用文字描述，如“y的λ次方”）

2. 数学公式、符号和希腊字母请按以下示例用文字描述：
   - "λ = 0" 而不是 "\lambda = 0"
   - "y(λ) = (y^λ - 1) / λ" 而不是 "\frac{y^\lambda - 1}{\lambda}"
   - "当 λ ≠ 0 时" 而不是 "\lambda \neq 0"
   - "z = (x - μ) / σ" 而不是 "z = \frac{x - \mu}{\sigma}"
...
async def main():
    # 要扫描的目录 (请修改为你的实际路径)
    target_dir = r"D:\work_space\python\langChain-demo\sklearn&matplot\sk-preprocessing_demo"  # 👈 改成你的目录
